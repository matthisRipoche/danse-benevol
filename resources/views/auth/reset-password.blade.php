...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Nouveau mot de passe — {{ config('app.name', 'Laravel') }}</title>
            ...
            <main class="w-full max-w-md rounded-2xl border border-sand-200 bg-white p-6 shadow-[0_2px_6px_-1px_rgba(92,25,17,0.04),0_1px_3px_-1px_rgba(92,25,17,0.02)]">
                <div class="relative mb-6 overflow-hidden rounded-xl bg-brand-600 p-6 text-white">
                    ...
                    <div class="relative flex flex-col gap-1">
                        ...
                        <h1 class="font-heading text-2xl font-bold">Nouveau mot de passe</h1>
                        <p class="text-sm text-white/80">Choisis un nouveau mot de passe pour ton compte.</p>
                    </div>
                </div>
                ...
                @if ($errors->any())
                    <div class="mb-6 rounded-lg border border-red-300 bg-red-50 p-4 text-sm text-red-700">
                        ...
                        @error('email')
                            <a href="{{ route('password.request') }}" class="mt-2 inline-block font-semibold text-red-800 underline">Demander un nouveau lien</a>
                        @enderror
                    </div>
                @endif
                ...
                <form method="POST" action="{{ route('password.store') }}" class="flex flex-col gap-4">
                    ...
                    @foreach (['password' => 'Nouveau mot de passe', 'password_confirmation' => 'Confirmer le mot de passe'] as $passwordField => $passwordLabel)
                        <div class="flex flex-col gap-1">
                            <label for="{{ $passwordField }}" class="text-sm font-medium text-stone-600">{{ $passwordLabel }}</label>
                            <div class="relative flex items-center">
                                <input type="password" name="{{ $passwordField }}" id="{{ $passwordField }}" required autocomplete="new-password" @if ($loop->first) autofocus @endif
