...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Nouvelle mission — {{ config('app.name', 'Laravel') }}</title>
    ...
    <body class="bg-sand-50 text-[#1e1e24] antialiased">
        ...
        <main class="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
            ...
            <div class="max-w-xl rounded-xl border border-sand-200 bg-white p-6">
                <h1 class="mb-5 font-heading text-xl font-bold">Nouvelle mission</h1>

                @include('admin.partials.flash')

                <form method="POST" action="{{ route('admin.missions.store') }}" class="flex flex-col gap-5">
                    @csrf

                    @include('admin.missions.partials.fields', [
                        'mission' => null,
                        'defaultCapacityHint' => $timeSlotCount > 0
                            ? "Appliqué aux {$timeSlotCount} créneau(x) de l'édition, puis ajustable créneau par créneau. 0 = aucun créneau ouvert."
                            : "Appliqué à chaque créneau dès qu'il sera créé. 0 = aucun créneau ouvert.",
                    ])
